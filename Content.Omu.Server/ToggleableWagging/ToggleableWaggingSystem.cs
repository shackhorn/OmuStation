...
/// </summary>
public sealed class ToggleableWaggingSystem : EntitySystem
{
    ...
    private void OnWaggingShutdown(EntityUid uid, ToggleableWaggingComponent component, ComponentShutdown args)
    {
        _actions.RemoveAction(uid, component.ActionEntity);
    }

    private void OnWaggingToggle(EntityUid uid, ToggleableWaggingComponent component, ref ToggleActionEvent args)
    {
        if (args.Handled)
            return;
    ...
    private void OnMobStateChanged(EntityUid uid, ToggleableWaggingComponent component, MobStateChangedEvent args)
    {
        if (component.Wagging)
            TryToggleWagging(uid, wagging: component);
    }

    public bool TryToggleWagging(EntityUid uid, ToggleableWaggingComponent? wagging = null, HumanoidAppearanceComponent? humanoid = null)
    {
    ...
    private bool HasTailWaggingMarkings(EntityUid uid, ToggleableWaggingComponent? wagging = null, HumanoidAppearanceComponent? humanoid = null)
    {
        if (!Resolve(uid, ref wagging, ref humanoid))
            return false;
